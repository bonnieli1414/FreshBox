...
public class z_repoVacations : BaseClass
{
    ...
    public string GetSQLSelect()
    {
        ...
        str_query = dpr.GetSQLSelectCommand(new Vacations());
        return str_query;
    }
    /// <summary>
    /// 取得模擬搜尋的欄位集合
    ...
    public List<string> GetSearchColumns()
    {
        ...
        //由系統自動取得文字欄位的集合
        using var dpr = new DapperRepository();
        searchColumn = dpr.GetStringColumnList(new Vacations());
        return searchColumn;
    }
    /// <summary>
    /// SQL 查詢條件式指令
    ...
    /// 預設 SQL 排序指令
    /// </summary>
    private readonly string DefaultOrderByColumn = "Vacations.DataNo";
    /// <summary>
    /// 預設 SQL 排序方式指令
    /// </summary>
    ...
    public List<SelectListItem> GetDropDownList(bool showNo = true)
    {
        ...
        str_query += "DataName AS Text , DataNo AS Value FROM Vacations ORDER BY DataNo";
    ...
    public string GetSQLInsert()
    {
        ...
        using var dpr = new DapperRepository();
        str_query = dpr.GetSQLInsertCommand(new Vacations());
        return str_query;
    }
    /// <summary>
    /// SQL 刪除指令
    ...
    public string GetSQLDelete()
    {
        ...
        using var dpr = new DapperRepository();
        str_query = dpr.GetSQLDeleteCommand(new Vacations());
        return str_query;
    }
    /// <summary>
    /// SQL 修改指令
    ...
    public string GetSQLUpdate()
    {
        ...
        using var dpr = new DapperRepository();
        str_query = dpr.GetSQLUpdateCommand(new Vacations());
        return str_query;
    }
    #endregion
    #region 物件建構子
    ...
    public z_repoVacations(string orderByColumn, string orderByDirection)
    {
    ...
    /// <returns></returns>
    public Vacations GetData(int id)
    {
        var model = new Vacations();
        if (id == 0)
        {
            //新增預設值
        }
        else
        {
            ...
            model = dpr.ReadSingle<Vacations>(sql_query, parm);
        }
        return model;
    }
    /// <summary>
    ...
    /// <returns></returns>
    public List<Vacations> GetDataList(string searchString = "")
    {
        List<string> searchColumns = GetSearchColumns();
        DynamicParameters parm = new DynamicParameters();
        var model = new List<Vacations>();
        using var dpr = new DapperRepository();
        string sql_query = GetSQLSelect();
        string sql_where = GetSQLWhere();
        sql_query += sql_where;
        if (!string.IsNullOrEmpty(searchString))
            sql_query += dpr.GetSQLWhereBySearchColumn(new Vacations(), searchColumns, sql_where, searchString);
        if (!string.IsNullOrEmpty(sql_where))
        {
            //自定義的 Weher Parm 參數
            //parm.Add("參數名稱", "參數值");
        ...
        sql_query += GetSQLOrderBy();
        model = dpr.ReadAll<Vacations>(sql_query, parm);
        return model;
    }
    /// <summary>
    /// 新增或修改資料(同步呼叫)
    ...
    /// </summary>
    /// <param name="model">資料模型</param>
    public void Create(Vacations model)
    {
        using var dpr = new DapperRepository();
    ...
    /// </summary>
    /// <param name="model">資料模型</param>
    public void Edit(Vacations model)
    {
        using var dpr = new DapperRepository();
    ...
    {
        using var dpr = new DapperRepository();
        string str_query = dpr.GetSQLDeleteCommand(new Vacations());
        DynamicParameters parm = dpr.GetSQLDeleteParameters(new Vacations(), id);
        dpr.Execute(str_query, parm);
    }
    #endregion
    ...
    /// <param name="id">Key 欄位值</param>
    /// <returns></returns>
    public async Task<Vacations> GetDataAsync(int id)
    {
        var model = new Vacations();
        if (id == 0)
        {
        ...
        else
        {
            ...
            model = await dpr.ReadSingleAsync<Vacations>(sql_query, parm);
        }
        return model;
    }
    /// <summary>
    ...
    /// <param name="searchString">模糊搜尋文字(空白或不傳入表示不搜尋)</param>
    /// <returns></returns>
    public async Task<List<Vacations>> GetDataListAsync(string searchString = "")
    {
        List<string> searchColumns = GetSearchColumns();
        DynamicParameters parm = new DynamicParameters();
        var model = new List<Vacations>();
       ...
       sql_query += sql_where;
        ...
        sql_query += GetSQLOrderBy();
        model = await dpr.ReadAllAsync<Vacations>(sql_query, parm);
        return model;
    }
    /// <summary>
    /// 新增或修改資料(非同步呼叫)
    ...
    /// <param name="model">資料模型</param>
    /// <returns></returns>
    public async Task CreateEditAsync(Vacations model)
    {
        if (model.Id == 0)
    ...
    /// <param name="model">資料模型</param>
    /// <returns></returns>
    public async Task CreateAsync(Vacations model)
    {
        using var dpr = new DapperRepository();
        string str_query = dpr.GetSQLInsertCommand(model);
        DynamicParameters parm = dpr.GetSQLInsertParameters(model);
    ...
    /// <param name="model">資料模型</param>
    /// <returns></returns>
    public async Task EditAsync(Vacations model)
    {
        using var dpr = new DapperRepository();
        string str_query = dpr.GetSQLUpdateCommand(model);
        DynamicParameters parm = dpr.GetSQLUpdateParameters(model);
