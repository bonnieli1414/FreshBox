...
public class z_repoLogs : BaseClass
{
    ...
    public string GetSQLSelect()
    {
        ...
        str_query = dpr.GetSQLSelectCommand(new Logs());
        return str_query;
    }
    /// <summary>
    /// 取得模擬搜尋的欄位集合
    ...
    public List<string> GetSearchColumns()
    {
        ...
        //由系統自動取得文字欄位的集合
        using var dpr = new DapperRepository();
        searchColumn = dpr.GetStringColumnList(new Logs());
        return searchColumn;
    }
    /// <summary>
    /// SQL 查詢條件式指令
    ...
    /// 預設 SQL 排序指令
    /// </summary>
    private readonly string DefaultOrderByColumn = "Logs.DataNo";
    /// <summary>
    /// 預設 SQL 排序方式指令
    /// </summary>
    ...
    public List<SelectListItem> GetDropDownList(bool showNo = true)
    {
        ...
        str_query += "DataName AS Text , DataNo AS Value FROM Logs ORDER BY DataNo";
    ...
    public string GetSQLInsert()
    {
        ...
        using var dpr = new DapperRepository();
        str_query = dpr.GetSQLInsertCommand(new Logs());
        return str_query;
    }
    /// <summary>
    /// SQL 刪除指令
    ...
    public string GetSQLDelete()
    {
        ...
        using var dpr = new DapperRepository();
        str_query = dpr.GetSQLDeleteCommand(new Logs());
        return str_query;
    }
    /// <summary>
    /// SQL 修改指令
    ...
    public string GetSQLUpdate()
    {
        ...
        using var dpr = new DapperRepository();
        str_query = dpr.GetSQLUpdateCommand(new Logs());
        return str_query;
    }
    #endregion
    #region 物件建構子
    ...
    public z_repoLogs(string orderByColumn, string orderByDirection)
    {
    ...
    public Logs GetData(int id)
    {
        ...
        else
        {
            ...
            model = dpr.ReadSingle<Logs>(sql_query, parm);
        }
        return model;
    }
    /// <summary>
    ...
    /// <returns></returns>
    public List<Logs> GetDataList(string searchString = "")
    {
        List<string> searchColumns = GetSearchColumns();
        DynamicParameters parm = new DynamicParameters();
        var model = new List<Logs>();
        using var dpr = new DapperRepository();
        string sql_query = GetSQLSelect();
        string sql_where = GetSQLWhere();
        sql_query += sql_where;
        if (!string.IsNullOrEmpty(searchString))
            sql_query += dpr.GetSQLWhereBySearchColumn(new Logs(), searchColumns, sql_where, searchString);
        if (!string.IsNullOrEmpty(sql_where))
        {
            //自定義的 Weher Parm 參數
            //parm.Add("參數名稱", "參數值");
        ...
        sql_query += GetSQLOrderBy();
        model = dpr.ReadAll<Logs>(sql_query, parm);
        return model;
    }
    /// <summary>
    /// 新增或修改資料(同步呼叫)
    ...
    {
        using var dpr = new DapperRepository();
        string str_query = dpr.GetSQLDeleteCommand(new Logs());
        DynamicParameters parm = dpr.GetSQLDeleteParameters(new Logs(), id);
        dpr.Execute(str_query, parm);
    }
    #endregion
    ...
    /// <param name="id">Key 欄位值</param>
    /// <returns></returns>
    public async Task<Logs> GetDataAsync(int id)
    {
        var model = new Logs();
        if (id == 0)
        {
        ...
        else
        {
            ...
            model = await dpr.ReadSingleAsync<Logs>(sql_query, parm);
        }
        return model;
    }
    /// <summary>
    ...
    /// <param name="searchString">模糊搜尋文字(空白或不傳入表示不搜尋)</param>
    /// <returns></returns>
    public async Task<List<Logs>> GetDataListAsync(string searchString = "")
    {
        List<string> searchColumns = GetSearchColumns();
        DynamicParameters parm = new DynamicParameters();
        var model = new List<Logs>();
       ...
       sql_query += sql_where;
        ...
        sql_query += GetSQLOrderBy();
        model = await dpr.ReadAllAsync<Logs>(sql_query, parm);
        return model;
    }
    /// <summary>
    /// 新增或修改資料(非同步呼叫)
    ...
    /// <param name="model">資料模型</param>
    /// <returns></returns>
    public async Task CreateEditAsync(Logs model)
    {
        if (model.Id == 0)
    ...
    /// <param name="model">資料模型</param>
    /// <returns></returns>
    public async Task CreateAsync(Logs model)
    {
        using var dpr = new DapperRepository();
        string str_query = dpr.GetSQLInsertCommand(model);
        DynamicParameters parm = dpr.GetSQLInsertParameters(model);
    ...
    /// <param name="model">資料模型</param>
    /// <returns></returns>
    public async Task EditAsync(Logs model)
    {
        using var dpr = new DapperRepository();
        string str_query = dpr.GetSQLUpdateCommand(model);
        DynamicParameters parm = dpr.GetSQLUpdateParameters(model);
