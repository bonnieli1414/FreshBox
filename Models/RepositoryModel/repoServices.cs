...
public class z_repoServices : BaseClass
{
    ...
    public string GetSQLSelect()
    {
        ...
        str_query = dpr.GetSQLSelectCommand(new Services());
        return str_query;
    }
    /// <summary>
    /// 取得模擬搜尋的欄位集合
    ...
    public List<string> GetSearchColumns()
    {
        ...
        //由系統自動取得文字欄位的集合
        using var dpr = new DapperRepository();
        searchColumn = dpr.GetStringColumnList(new Services());
        return searchColumn;
    }
    /// <summary>
    /// SQL 查詢條件式指令
    ...
    /// 預設 SQL 排序指令
    /// </summary>
    private readonly string DefaultOrderByColumn = "Services.DataNo";
    /// <summary>
    /// 預設 SQL 排序方式指令
    /// </summary>
    ...
    public List<SelectListItem> GetDropDownList(bool showNo = true)
    {
        ...
        str_query += "DataName AS Text , DataNo AS Value FROM Services ORDER BY DataNo";
    ...
    public string GetSQLInsert()
    {
        ...
        using var dpr = new DapperRepository();
        str_query = dpr.GetSQLInsertCommand(new Services());
        return str_query;
    }
    /// <summary>
    /// SQL 刪除指令
    ...
    public string GetSQLDelete()
    {
        ...
        using var dpr = new DapperRepository();
        str_query = dpr.GetSQLDeleteCommand(new Services());
        return str_query;
    }
    /// <summary>
    /// SQL 修改指令
    ...
    public string GetSQLUpdate()
    {
        ...
        using var dpr = new DapperRepository();
        str_query = dpr.GetSQLUpdateCommand(new Services());
        return str_query;
    }
    #endregion
    #region 物件建構子
    ...
    public z_repoServices(string orderByColumn, string orderByDirection)
    {
    ...
    /// <returns></returns>
    public Services GetData(int id)
    {
        ...
        else
        {
            ...
            model = dpr.ReadSingle<Services>(sql_query, parm);
        }
        return model;
    }
    /// <summary>
    ...
    /// <returns></returns>
    public List<Services> GetDataList(string searchString = "")
    {
        List<string> searchColumns = GetSearchColumns();
        DynamicParameters parm = new DynamicParameters();
        var model = new List<Services>();
        using var dpr = new DapperRepository();
        string sql_query = GetSQLSelect();
        string sql_where = GetSQLWhere();
        sql_query += sql_where;
        if (!string.IsNullOrEmpty(searchString))
            sql_query += dpr.GetSQLWhereBySearchColumn(new Services(), searchColumns, sql_where, searchString);
        if (!string.IsNullOrEmpty(sql_where))
        {
            //自定義的 Weher Parm 參數
            //parm.Add("參數名稱", "參數值");
        ...
        sql_query += GetSQLOrderBy();
        model = dpr.ReadAll<Services>(sql_query, parm);
        return model;
    }
    /// <summary>
    /// 新增或修改資料(同步呼叫)
    ...
    /// </summary>
    /// <param name="model">資料模型</param>
    public void Create(Services model)
    {
        using var dpr = new DapperRepository();
    ...
    /// </summary>
    /// <param name="model">資料模型</param>
    public void Edit(Services model)
    {
        using var dpr = new DapperRepository();
    ...
    {
        using var dpr = new DapperRepository();
        string str_query = dpr.GetSQLDeleteCommand(new Services());
        DynamicParameters parm = dpr.GetSQLDeleteParameters(new Services(), id);
        dpr.Execute(str_query, parm);
    }
    #endregion
    ...
    /// <param name="id">Key 欄位值</param>
    /// <returns></returns>
    public async Task<Services> GetDataAsync(int id)
    {
        var model = new Services();
        if (id == 0)
        {
        ...
        else
        {
            ...
            model = await dpr.ReadSingleAsync<Services>(sql_query, parm);
        }
        return model;
    }
    /// <summary>
    ...
    /// <param name="searchString">模糊搜尋文字(空白或不傳入表示不搜尋)</param>
    /// <returns></returns>
    public async Task<List<Services>> GetDataListAsync(string searchString = "")
    {
        List<string> searchColumns = GetSearchColumns();
        DynamicParameters parm = new DynamicParameters();
        var model = new List<Services>();
       ...
       sql_query += sql_where;
        ...
        sql_query += GetSQLOrderBy();
        model = await dpr.ReadAllAsync<Services>(sql_query, parm);
        return model;
    }
    /// <summary>
    /// 新增或修改資料(非同步呼叫)
    ...
    /// <param name="model">資料模型</param>
    /// <returns></returns>
    public async Task CreateEditAsync(Services model)
    {
        if (model.Id == 0)
    ...
    /// <param name="model">資料模型</param>
    /// <returns></returns>
    public async Task CreateAsync(Services model)
    {
        using var dpr = new DapperRepository();
        string str_query = dpr.GetSQLInsertCommand(model);
        DynamicParameters parm = dpr.GetSQLInsertParameters(model);
    ...
    /// <param name="model">資料模型</param>
    /// <returns></returns>
    public async Task EditAsync(Services model)
    {
        using var dpr = new DapperRepository();
        string str_query = dpr.GetSQLUpdateCommand(model);
        DynamicParameters parm = dpr.GetSQLUpdateParameters(model);
