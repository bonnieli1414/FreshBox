...
/// 訂單資料 CRUD 程式
/// </summary>
public class z_repoOrderUnClose : BaseClass
{
    #region SQL 指令設定區
    /// <summary>
    /// SQL 查詢欄位及表格指令
...
Shippings.ShippingName,Orders.ReceiverName,Orders.ReceiverEmail,Orders.ReceiverAddress,Orders.OrderAmount,Orders.TaxAmount,Orders.TotalAmount,Orders.Remark,Orders.GuidNo";
 ...
 LEFT OUTER JOIN dbo.Shippings ON Orders.ShippingNo = Shippings.ShippingNo ";
    ...
    {
        string str_query = " WHERE Orders.IsClosed = 0 ";
        return str_query;
    }
    /// <summary>
    /// 預設 SQL 排序指令
    ...
    public z_repoOrderUnClose(string orderByColumn, string orderByDirection)
    {
    ...
    public Orders GetData(string sheetNo)
    {
        ...
        sql_query += " WHERE Orders.SheetNo = @SheetNo AND Orders.IsClosed = 0";
    ...
    public void Edit(Orders model)
    {
        ...
        string str_query = "UPDATE Orders SET StatusCode = @StatusCode WHERE Id = @Id";
        DynamicParameters parm = new DynamicParameters();
        parm.Add("StatusCode", model.StatusName);
