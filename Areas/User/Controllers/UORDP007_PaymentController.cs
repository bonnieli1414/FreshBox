...
{
    public class UORDP007_PaymentController : Controller
    {
        /// <summary>
        /// 資料初始化事件
        /// </summary>
        ...
        {
            using var Payments = new z_repoPayments(SessionService.SortColumn, SessionService.SortDirection);
            var model = Payments.GetDataList(SessionService.SearchText)
        ...
        public IActionResult CreateEdit(int id = 0)
        {
            ...
            {
                //修改資料
                using var Payments = new z_repoPayments();
                model = Payments.GetData(id);
        ...
        [Login(RoleList = "User")]
        [HttpPost]
        public IActionResult CreateEdit(Payments model)
        {
            if (!ModelState.IsValid) return View(model);
            using var Payments = new z_repoPayments();
            Payments.CreateEdit(model);
            return RedirectToAction("Index", ActionService.Controller, new { area = ActionService.Area });
        }
