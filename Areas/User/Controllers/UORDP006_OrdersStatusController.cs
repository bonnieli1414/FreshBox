...
{
    public class UORDP006_OrdersStatusController : Controller
    {
        /// <summary>
        /// 資料初始化事件
        /// </summary>
        ...
        {
            using var OrdersStatus = new z_repoOrdersStatus(SessionService.SortColumn, SessionService.SortDirection);
            var model = OrdersStatus.GetDataList(SessionService.SearchText)
        ...
        public IActionResult CreateEdit(int id = 0)
        {
            ...
            {
                //修改資料
                using var OrdersStatus = new z_repoOrdersStatus();
                model = OrdersStatus.GetData(id);
        ...
        [Login(RoleList = "User")]
        [HttpPost]
        public IActionResult CreateEdit(OrdersStatus model)
        {
            if (!ModelState.IsValid) return View(model);
            using var OrdersStatus = new z_repoOrdersStatus();
            OrdersStatus.CreateEdit(model);
            return RedirectToAction("Index", ActionService.Controller, new { area = ActionService.Area });
        }
        ...
        public JsonResult DeleteRow(int id = 0)
        {
            ...
            using (z_repoOrdersStatus OrdersStatus = new z_repoOrdersStatus())
            {
                OrdersStatus.Delete(id);
